...
describe '課題の優先順位変更' do
  context do
    ...
    let(:issue_a) { create_issue(user_a, project_a, title: 'A') }
    let(:issue_b) { create_issue(user_a, project_a, title: 'B') }
    let(:issue_c) { create_issue(user_a, project_a, title: 'C') }

    let(:issue_x) { create_issue(user_b, project_b, title: 'X') }
    let(:issue_y) { create_issue(user_b, project_b, title: 'Y') }
    let(:issue_z) { create_issue(user_b, project_b, title: 'Z') }
